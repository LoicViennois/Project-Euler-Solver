import {
  areDisjoint,
  arePermutations, arraysEqual, concat, difference,
  digits2number,
  haveDuplicates,
  haveSameLength, intersection, maxBy,
  nbOccurrences,
  number2digits, range, transpose, union, uniq,
  wordValue, zip
} from './utils';
...
describe('Utils', () => {
  ...
  describe('maxBy', () => {
    test('maxBy length of [1, 2] and [1, 2, 3]', () => {
      expect(maxBy([[1, 2], [1, 2, 3]], (a) => a.length)).toEqual([1, 2, 3]);
    });
  });
